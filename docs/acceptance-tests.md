...
| `AT-056` | `CON-007` | `bun run mutation-check`가 커밋된 mutation 집합을 전부 돌려 각각이 잡히는지 보고한다. 잡히지 않으면 결함 설명과 함께 `NOT CAUGHT`으로 이름을 찍고 실패한다. 매치하지 않는 패턴은 통과가 아니라 `REFUSED`로 실패한다. |
| `AT-059` | `CON-009` | README가 밝히는 계약 버전이 `package.json`이 설치하는 태그와 같다. 어느 한쪽만 올리면 실패하고, README가 계약을 아예 언급하지 않게 되어도 실패한다. |
| `AT-057` | `CON-007` | `bun run mutation-check --self-check`이 무해한 변이와 매치하지 않는 패턴을 각각 **선언된 방식으로** 실패하는지 대조한다. `caught` 분기를 강제하면 self-check 자신이 실패하고, 첫 항목의 기준 문자열이 한 단어 바뀌어 `REFUSED`가 되어도 실패한다 — 개수만 세면 그때 2/2로 통과한다. |
| `AT-058` | `CON-007` | mutation 안에서 mutation-check를 돌리면 시작 단계에서 거부한다. 안쪽 실행은 바깥 편집을 더러운 트리로 보고 모든 항목을 거부하는데, 거부는 실패로 세므로 돌지 않은 self-check가 전부 정상으로 보고된다. |
| `AT-055` | `CON-009` | 파일 열거가 아무것도 못 찾으면 별도 케이스가 실패한다. 커버리지 검사는 파일 집합과 패턴 집합을 대조하므로 어느 쪽이 비어도 green이 되며, 양쪽 다 막아야 한다. |
| `AT-054` | `CON-009` | include에 `**/*.ts` 같은 전면 패턴이 들어오면 별도 케이스가 실패한다. 범위 테스트가 자명하게 통과하는 설정을 스스로 잡는다. |
| `AT-053` | `CON-009` | `tsconfig` include에서 디렉터리 하나를 빼면 `typecheck-scope` 테스트가 빠진 파일 이름을 찍고 실패한다. 규칙은 문서가 아니라 이 테스트가 강제한다. |
| `AT-052` | `CON-009` | 계약에 verb가 추가되고 러너가 처리하지 않으면 mesh를 띄우기 전에 `bun run check`가 실패한다. 동사 case를 지우면 `Type '{ do: "sleep"; … }' is not assignable to type 'never'`가 난다. |
...
| `AT-047` | `CON-006` | 하니스가 요구한 lease를 적용하지 않으면 시나리오는 통과가 아니라 시작 단계에서 멈춘다. 요구와 다른 lease가 적용되면 `E2E-CAP-001`이 실패한다. |
| `AT-048` | `CON-007` | `expect.body` 치환을 끄면 `provision` step에 달린 단언이 `E2E-KEY-003`에서 실패한다. 검사가 `http` 동사에만 있으면 통과했을 자리다. |
| `AT-049` | `CON-008` | `bind`를 무효화하면 `E2E-RECALL-001`이 `no binding for {{taken}}`으로 실패한다. 원문이 전송돼 404로 통과하지 않는다. |
| `AT-050` | `CON-002` | `as: { signedBy }`를 무시하면 `E2E-RECALL-001`이 401로 실패한다. 서명된 비관리자 호출이 실제로 그 신원으로 나간다. |
